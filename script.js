document.addEventListener('DOMContentLoaded', () => {
    ...
    els.excelInput.addEventListener('change', (e) => {
        ...
        reader.onload = (e) => {
            ...
            if (newStudents.length > 0) { appState.history.push([...appState.students]); els.undoBtn.disabled = false; appState.students = [...appState.students, ...newStudents]; renderReport(); showToast('Excel', `Đã nhập ${newStudents.length} dòng.`); }
        };
    ...
    window.deleteRow = id => { appState.history.push([...appState.students]); els.undoBtn.disabled = false; appState.students = appState.students.filter(s => s.id != id); renderReport(); };

    els.undoBtn.addEventListener('click', () => { if (appState.history.length > 0) { appState.students = appState.history.pop(); if (appState.history.length === 0) els.undoBtn.disabled = true; renderReport(); showToast('Undo', 'Đã hoàn tác.'); } });
    els.clearBtn.addEventListener('click', () => { if (confirm('Xóa toàn bộ?')) { appState.history.push([...appState.students]); els.undoBtn.disabled = false; appState.students = []; renderReport(); } });
    ...
    els.exportPngBtn.addEventListener('click', () => {
        ...
        exportDiv.innerHTML = `<div style="border: 2px solid #e5e7eb; border-radius: 12px; overflow: hidden; box-shadow: 0 10px 15px -3px rgba(0, 0, 0, 0.1);"><div style="background-color: #0d0deb; color: white; padding: 25px 20px; text-align: center;"><div style="font-size: 32px; margin-bottom: 5px;">🏫</div><h2 style="margin: 0; font-size: 18px; font-weight: 600; text-transform: uppercase; letter-spacing: 1px; opacity: 0.9;">TRUNG TÂM GDTX - NN, TH TỈNH LÂM ĐỒNG</h2><h1 style="margin: 10px 0 0; font-size: 24px; font-weight: 800; text-transform: uppercase;">Phiếu Ghi Nhận Vi Phạm</h1></div><div style="padding: 20px; background-color: #f8fafc; border-bottom: 2px solid #e5e7eb; font-size: 13px;"><div style="display: grid; grid-template-columns: 1fr 1fr; gap: 10px;"><div><span style="color: #64748b; font-weight: 600;">Mã:</span> <span style="font-weight: 700;">#${Math.floor(100000 + Math.random() * 900000)}</span></div><div style="text-align: right;"><span style="color: #64748b; font-weight: 600;">Ngày:</span> <span style="font-weight: 700;">${new Date().toLocaleDateString('vi-VN')}</span></div><div><span style="color: #64748b; font-weight: 600;">Giám thị:</span> <span style="font-weight: 700; text-transform: uppercase;">${appState.monitorName}</span></div><div style="text-align: right;"><span style="color: #64748b; font-weight: 600;">Lớp trực:</span> <span style="font-weight: 700;">${appState.monitorClass}</span></div></div></div><div style="padding: 20px;">${groupsHtml}</div><div style="background-color: #f8fafc; padding: 15px 20px; border-top: 2px solid #e2e8f0; display: flex; justify-content: space-between; align-items: center;"><span style="font-weight: 600; color: #475569; text-transform: uppercase; font-size: 13px;">Tổng số vi phạm</span><span style="font-size: 24px; font-weight: 800; color: #1e40af;">${appState.students.length}</span></div></div>`;
    ...
    els.exportExcelBtn.addEventListener('click', () => {
        ...
        const wsData = [['TRUNG TÂM GDTX - NN, TH TỈNH LÂM ĐỒNG', '', '', ''], ['DANH SÁCH VI PHẠM', '', '', ''], [`Giám thị: ${appState.monitorName}`, `Lớp: ${appState.monitorClass}`, '', ''], [`Ngày: ${new Date().toLocaleString('vi-VN')}`, '', '', ''], ['', '', '', ''], ['STT', 'Họ và Tên', 'Lớp', 'Lỗi Vi Phạm']];
